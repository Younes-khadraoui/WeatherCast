  ...
  if (data)
    return (
      <motion.div
        ...
        className="Main grid place-content-center bg-[#4682B4] p-5 rounded-lg "
      >
        ...
        <div className="flex justify-center items-center gap-8 text-4xl py-10">
          <p>{(data as Weather)?.main.temp}°C</p>
          <img src={iconSrc} alt="icon" />
        </div>
        <div className="flex justify-around flex-wrap">
          <div>
            <p className="text-xl">Humidity</p>
            <p className="text-center">{(data as Weather).main.humidity}%</p>
          </div>
          <div>
            <p className="text-xl">Air Pressure</p>
            <p className="text-center">{(data as Weather).main.pressure}hPa</p>
          </div>
          <div>
            <p className="text-xl">Wind</p>
            <p className="text-center">{(data as Weather).wind.speed}mph</p>
          </div>
        </div>
      </motion.div>
    );
